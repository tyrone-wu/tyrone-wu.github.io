...
The PVE management host and services are connected on `vmbr0`, which is linked to `enp0s25`.
...
I found that ICS configured on `Ethernet 3` also provided internet access, however, ICS was a little wonky on
reboots so I settled with NAT.
...
It's a janky setup, but it works ¯\\\_(ツ)\_/¯.

</section> {/* hyper-v-network */}

<br /><br />

<section id="previous-network-setup" class="scroll-anchor-offset">

<Accordion
  buttonClasses="w-full flex items-center hover:underline hover:font-bold border-b-1"
  iconClasses="size-6 ml-auto mr-2"
  contentsClasses="border-r-1 line-highlight"
>
<h2 slot="title" class="mt-2 mb-1 !border-b-0 !no-underline">Previous Network Setup</h2>
<span slot="contents">
#### 0.0.1 -- 0.0.3
...
</section> {/* previous-network-setup */}
