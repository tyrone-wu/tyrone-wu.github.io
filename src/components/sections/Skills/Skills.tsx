...
export default function Skills({ textPalette }: { textPalette: string }) {
  ...
  const innerBackground = useColorModeValue(
    "cardBackground.0",
    "cardBackground.1",
  )
  ...
  return (
    <Box id="skills">
      <Card textColor={textPalette} title="Skills">
        <SimpleGrid columns={[1, 2]} gap={6} mb={2}>
          <GridItem
...
function SkillSet({
  ...
  return (
    <Flex direction="column" mx={4} my={2}>
      ...
      <Wrap spacing="2px" justify="center" mb={2}>
        {skillSet.map((skill: string) => (
          <WrapItem key={skill}>
            <SkillIcon
              borderColor={textPalette}
              bgColor={skillsBGColor}
              skill={skill}
              isCert={isCert}
            />
          </WrapItem>
