import { Box, Center, Divider, Flex, Image, Link, SimpleGrid, Text, useColorModeValue } from "@chakra-ui/react";
  ...
  const projects = [
    {
      ...
      description: "An automated kubernetes, multi-node cluster deployment for an inventory web app.",
    ...
    },
    {
      ...
      description: "A Stack Exchange-like web app that hosting a crowd-reviewing platform for research publications.",
    ...
    {
      name: "One Wallet",
      description: "A web app that centralizes a user's personal finance information with modular and customizable components/interface.",
  ...
  return (
    <MiniCard textPalette={textPalette}>
      ...
      >
        ...
        <Text textStyle="sub" style={{textDecoration: "none"}}  w="full" mb={4}>
          {project.description}
        </Text>
        <Box
          borderWidth="1px"
        ...
        >
          {project.stack.map((tech: string, i: number) => (
            <TechIcon key={`${i}` + "-" + `${tech}`} tech={tech} textPalette={textPalette} iconBGColor={iconBGColor} />
          ))}
        </Flex>
      </Flex>
    </MiniCard>
...
function TechIcon({ tech, textPalette, iconBGColor }: { tech: string, textPalette: string, iconBGColor: string }) {
  ...
  return (
      ...
      >
        <Center mx="6px">
          <Image src={stack?.icon} alt={tech} w="20px" mr="4px" />
          <Text textStyle="icon">
            {tech}
          </Text>
        </Center>
